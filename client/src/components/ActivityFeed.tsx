...
function FileStatusIcon({ status }: { status: string }) {
	if (status === "added")
		return <FilePlus size={10} className="text-green-400 shrink-0" />;
	if (status === "removed")
		return <FileMinus size={10} className="text-red-400 shrink-0" />;
...
function FileStatusBadge({ status }: { status: string }) {
	...
	return (
		<span className={`text-[9px] font-bold px-1 rounded font-mono ${s.cls}`}>
			{s.label}
		</span>
	);
}
	...
	const loadDetail = useCallback(async () => {
		...
		try {
			...
			const res = await fetch(
				`${API_BASE}/api/projects/${projectId}/commits/${commit.sha}`,
				{
							...
							{loadingDetail ? (
								...
								<div className="p-2 space-y-px">
									...
									{detail.files.map((f) => (
										<div
											key={f.filename}
											className="flex items-center gap-1.5 px-1 py-0.5 rounded hover:bg-[#27272a] transition-colors"
										>
											...
											<span className="text-[9px] font-mono text-green-400/70 shrink-0">
												+{f.additions}
											</span>
											<span className="text-[9px] font-mono text-red-400/70 shrink-0">
												-{f.deletions}
											</span>
										</div>
									))}
								...
								<p className="text-[10px] text-zinc-600 p-2.5">
		...
		<div className="flex flex-col h-full bg-[#09090b] text-[#c9d1d9] text-sm font-sans relative">
			...
			<div className="flex items-center justify-between p-3 border-b border-[#27272a] shrink-0 sticky top-0 bg-[#09090b] z-20">
				<div className="flex items-center gap-2">
					...
					<h2 className="font-semibold text-xs tracking-widest uppercase text-gray-300">
						Commit History
					</h2>
				</div>
				<button
