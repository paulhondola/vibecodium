...
export default function ReactionOverlay({
	...
	const spawnEmoji = (emoji: string, sender?: string) => {
		...
		setFloating((prev) => [
			...prev,
			{ id, emoji, x, startTime: Date.now(), sender },
		]);
		...
		<>
			...
			<div className="fixed inset-0 pointer-events-none z-[999] overflow-hidden">
				{floating.map((item) => (
					...
					>
						{item.emoji}
						{item.sender && (
							<div className="text-[9px] text-white/60 font-mono text-center mt-1">
								{item.sender}
							</div>
						)}
					</div>
			...
			<div className="relative">
				<button
					...
					className={
						buttonClassName ??
						"text-xs px-3 py-1.5 rounded flex items-center gap-2 transition-all font-semibold shadow-sm bg-purple-900/30 text-purple-400 border border-purple-500/30 hover:bg-purple-500/20 hover:scale-105"
					}
				...
				{showPicker && (
					<div
						className={`absolute ${pickerPosition === "above" ? "bottom-full mb-2" : "top-full mt-2"} right-0 bg-[#18181b] border border-[#3f3f46] rounded-xl p-3 shadow-2xl shadow-black/60 grid grid-cols-5 gap-2 z-50 min-w-[200px]`}
					>
