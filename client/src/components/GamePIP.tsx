...
export default function GamePIP({
	onClose,
	gameType,
	onSwitchGame,
}: GamePIPProps) {
	...
	const handleMouseMove = (e: MouseEvent) => {
		if (isDragging) {
			...
			const newWidth = Math.max(
				280,
				Math.min(resizeStart.width + deltaX, window.innerWidth - position.x),
			);
			const newHeight = Math.max(
				200,
				Math.min(resizeStart.height + deltaY, window.innerHeight - position.y),
			);
		...
		>
			...
			>
				...
				<div className="flex items-center gap-1">
					<button
						onClick={() =>
							onSwitchGame(gameType === "subway" ? "flappy" : "subway")
						}
