...
export default function EditorArea({
	...
	useEffect(() => {
		if (gameOpen) setActiveGame(initialGameType);
	}, [gameOpen, initialGameType]);
	...
	const contentListenerDisposable = useRef<{ dispose: () => void } | null>(
		null,
	);
	...
	useEffect(() => {
		const handleKeyDown = (e: KeyboardEvent) => {
			...
			if (
				e.key === konamiCode[konamiIndex.current] ||
				e.key.toLowerCase() === konamiCode[konamiIndex.current].toLowerCase()
			) {
				konamiIndex.current++;
				if (konamiIndex.current === konamiCode.length) {
					...
					new Audio("https://www.myinstants.com/media/sounds/1up.mp3")
						.play()
						.catch(() => {});
				}
			} else {
	...
	useEffect(() => {
		...
		if (activeFileRef.current)
			activeFileRef.current.content = remoteCodeUpdate.content;
	...
	const applyRemoteCursors = () => {
		if (!decorationsRef.current || !monacoRef.current || !editorRef.current)
			return;
		const ranges = Array.from(remoteCursorsRef.current.entries()).map(
			([clientId, { color, userName, position }]) => {
	...
	return (
		<div className="flex flex-col h-full bg-[#09090b] text-[#c9d1d9] relative">
			...
			<div className="flex bg-[#18181b] border-b border-[#27272a] shrink-0 overflow-x-auto no-scrollbar scroll-smooth">
				{openFiles.map((file) => {
					...
					return (
						...
						>
							<span
								className={`font-bold text-[10px] ${isActive ? (isJS ? "text-yellow-400" : "text-zinc-400") : "text-zinc-600"}`}
							>
			...
			{powerModeEnabled && combo > 10 && (
				<div
					className={`absolute top-12 right-4 z-40 pointer-events-none flex flex-col items-end gap-1 transition-all duration-200 ${isPowerMode ? "opacity-100" : "opacity-70"}`}
				>
					<div
					...
					<div
						className={`text-4xl font-black font-mono ${isPowerMode ? "text-orange-400 drop-shadow-[0_0_8px_rgba(249,115,22,1)]" : "text-gray-400"}`}
					>
						x{combo}
					</div>
				</div>
				...
				{activeFile ? (
					...
					<div className="flex h-full w-full items-center justify-center bg-[#09090b] select-none">
						<div className="text-center flex flex-col items-center">
							<div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-purple-600 to-purple-900 flex items-center justify-center text-white text-3xl font-bold mb-6">
								VC
							</div>
							<h2 className="text-2xl font-semibold text-zinc-200 mb-2 tracking-[-0.48px]">
								VibeCodium Editor
							</h2>
							...
							<div className="flex flex-col items-start text-xs text-zinc-500 gap-3 font-mono border-t border-[#27272a] pt-6">
								<div className="flex items-center justify-between w-64">
									<span className="text-zinc-600">Go to File</span>{" "}
									<span className="px-1.5 py-0.5 rounded bg-purple-500/10 border border-purple-500/20 text-purple-400">
										⌘ P
									</span>
								</div>
								<div className="flex items-center justify-between w-64">
									<span className="text-zinc-600">Search in Files</span>{" "}
									<span className="px-1.5 py-0.5 rounded bg-purple-500/10 border border-purple-500/20 text-purple-400">
										⌘ ⇧ F
									</span>
								</div>
								<div className="flex items-center justify-between w-64">
									<span className="text-zinc-600">Show Explorer</span>{" "}
									<span className="px-1.5 py-0.5 rounded bg-[#18181b] border border-[#27272a] text-zinc-400">
										⌘ E
									</span>
								</div>
								<div className="flex items-center justify-between w-64">
									<span className="text-zinc-600">Toggle Terminal</span>{" "}
									<span className="px-1.5 py-0.5 rounded bg-[#18181b] border border-[#27272a] text-zinc-400">
										⌘ J
									</span>
								</div>
								<div className="flex items-center justify-between w-64">
									<span className="text-zinc-600">Toggle Agent Chat</span>{" "}
									<span className="px-1.5 py-0.5 rounded bg-[#18181b] border border-[#27272a] text-zinc-400">
										⌘ B
									</span>
								</div>
								<div className="flex items-center justify-between w-64">
									<span className="text-zinc-500">Toggle Zen Mode</span>{" "}
									<span className="px-1.5 py-0.5 rounded bg-purple-500/10 border border-purple-500/20 text-purple-400">
										⌘ K
									</span>
								</div>
					...
					<div className="absolute bottom-4 right-4 left-4 max-w-[480px] ml-auto max-h-[55%] bg-[#18181b] border border-purple-500/30 rounded-[10px] overflow-hidden flex flex-col z-30">
						...
						<div className="flex items-center justify-between px-3 py-2 bg-purple-900/20 border-b border-purple-500/20 shrink-0">
							<div className="flex items-center gap-2 text-purple-300 font-medium text-[11px]">
								...
								<span>
									Suggested change in{" "}
									<code className="text-purple-200 bg-purple-500/10 px-1 py-0.5 rounded font-mono text-[10px]">
										{pendingUpdate.filePath.split("/").pop()}
									</code>
								</span>
							</div>
						...
						<div className="overflow-auto p-3 font-mono text-[11px] leading-5 flex-1">
							...
							{diffLines(
								...
								>
									<span
										className={`shrink-0 select-none w-3 text-center ${line.type === "remove" ? "text-red-600" : "text-green-600"}`}
									>
										{line.type === "remove" ? "−" : "+"}
									</span>
						...
						<div className="shrink-0 px-3 py-1.5 border-t border-purple-500/10 text-[9px] text-gray-600 flex items-center justify-between">
							...
							<kbd className="px-1 py-0.5 bg-[#18181b] border border-[#27272a] rounded text-gray-500">
								Esc
							</kbd>
						</div>
					</div>
			...
			<div className="shrink-0 h-6 bg-[#111113] border-t border-[#1f1f24] flex items-center px-3 gap-4 text-[10px] font-mono text-zinc-500 select-none">
				...
				{activeFile && (
					<>
						...
						<span className="truncate text-zinc-700 ml-auto">
			...
			{contextMenu && (
				...
				>
					...
					<button
						className="w-full text-left px-3 py-1.5 text-xs text-zinc-300 hover:bg-[#27272a] transition-colors"
						onClick={() => {
							onCloseOthers?.(contextMenu.file);
							setContextMenu(null);
						}}
					>
						Close Others
