...
const getIcon = (name: string) => {
	...
	if (name.endsWith(".json"))
		return <FileJson size={13} className="text-green-400 shrink-0" />;
	if (name.endsWith(".md"))
...
function ContextMenu({
	...
	const btn =
		"flex items-center gap-2 w-full px-3 py-1.5 text-xs text-gray-300 hover:bg-[#27272a] hover:text-white transition-colors rounded text-left";
			...
			{!menu.node.isFile && (
				<>
					<button className={btn} onClick={onNewFile}>
						<FilePlus size={13} className="text-zinc-400" /> New File
					</button>
					<button className={btn} onClick={onNewFolder}>
						<FolderPlus size={13} className="text-zinc-400" /> New Folder
					</button>
					...
					<div
						...
						className={`flex items-center gap-1.5 px-3 py-[3px] cursor-pointer select-none transition-colors border-l-2 group ${
							isActive
								...
								: "border-transparent hover:bg-[#18181b] text-zinc-400 hover:text-white"
						}`}
						style={{ paddingLeft: `calc(0.75rem + ${depth * 0.75}rem)` }}
					>
						{getIcon(node.name)}
						...
						<button
							onClick={(e) => {
								e.stopPropagation();
								actions.onDelete(node.path, true);
							}}
				...
				>
					...
					<div className="ml-auto flex items-center gap-0.5 opacity-0 group-hover:opacity-100">
						<button
							...
							className="p-0.5 hover:text-purple-400 text-gray-500 rounded transition-colors"
							title="New file"
						>
							<FilePlus size={11} />
						</button>
						...
						<button
							onClick={(e) => {
								e.stopPropagation();
								actions.onDelete(node.path, false);
							}}
							className="p-0.5 hover:text-red-400 text-gray-500 rounded transition-colors"
							title="Delete folder"
						>
							<Trash2 size={11} />
						</button>
...
export default function FileExplorer({
	...
	const [rootInline, setRootInline] = useState<null | "newFile" | "newFolder">(
		null,
	);
	...
	const doDelete = useCallback(
		async (path: string, _isFile: boolean) => {
	...
	const doRename = useCallback(
		async (oldPath: string, newPath: string, _isFile: boolean) => {
	...
	return (
		<div className="flex flex-col h-full bg-[#09090b] text-[#c9d1d9] text-[13px] font-mono leading-relaxed select-none">
			...
			<div className="flex items-center justify-between px-3 py-2 border-b border-[#27272a] shrink-0 font-sans">
				<h2 className="font-semibold text-[10px] tracking-wider uppercase text-gray-500">
					Explorer
				</h2>
				{!readOnly && (
			...
			<div className="flex-1 overflow-y-auto py-1 scrollbar-thin scrollbar-thumb-[#27272a] scrollbar-track-transparent">
				{(!files || files.length === 0) && !rootInline && (
					<div className="px-4 py-3 text-[11px] text-gray-600 italic font-sans">
						No files. Use <strong className="text-gray-400">+</strong> to create
						one.
					</div>
				)}
