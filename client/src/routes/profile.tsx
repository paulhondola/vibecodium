...
function ProfilePage() {
	...
	const [githubCommitCount, setGithubCommitCount] = useState<number | null>(
		null,
	);
		...
		if (user?.nickname) {
			...
			getAccessTokenSilently()
				.then((token) => {
					...
					return Promise.all([
						fetch(`${API_BASE}/api/github/users/${user.nickname}`, {
							headers,
						}).then((res) => res.json()),
						fetch(
							`${API_BASE}/api/github/search/commits?q=author:${user.nickname}`,
							{ headers },
						).then((res) => res.json()),
					]);
				})
				.then(([userData, commitData]) => {
					if (userData.public_repos !== undefined)
						setGithubRepoCount(userData.public_repos);
					if (commitData.total_count !== undefined)
						setGithubCommitCount(commitData.total_count);
				})
			...
			getAccessTokenSilently()
				...
				.then((data) => {
					if (data.success) {
						...
						const apiKeys = data.llmApiKeys || {};
						setLlmApiKeys(apiKeys);
						if (apiKeys[llmProvider]) {
							setLlmApiKey(apiKeys[llmProvider]);
						} else {
	...
	const handleSaveTokens = async () => {
		...
		try {
			...
			if (data.success) {
				...
				if (updatedTokens.githubToken)
					setGithubToken(updatedTokens.githubToken);
				if (updatedTokens.vercelToken)
					setVercelToken(updatedTokens.vercelToken);
				...
				const apiKeys = updatedTokens.llmApiKeys || {};
				setLlmApiKeys(apiKeys);
				setLlmApiKey(apiKeys[llmProvider] ?? "");
			} else {
				setSaveMessage({
	...
	const handleSaveProfile = async () => {
		...
		try {
			...
			const data = await res.json();
			setProfileSaveMsg({
				text: data.success ? "Profile updated!" : data.error || "Failed",
	...
	return (
		<div className="min-h-screen bg-[#02040a] text-[#f8fafc] font-sans selection:bg-[rgba(168,85,247,0.3)] overflow-x-hidden relative py-20 px-4 pt-24">
			...
			<motion.div
				...
				className="w-full max-w-5xl mx-auto grid md:grid-cols-3 gap-8 relative z-10"
			>
				{/* ── Left column ── */}
				<div className="md:col-span-1 flex flex-col gap-6">
					...
					<div className="bg-[rgba(10,12,20,0.6)] backdrop-blur-xl border border-[rgba(168,85,247,0.2)] rounded-2xl p-6 shadow-[0_0_30px_rgba(0,0,0,0.5)] relative overflow-hidden group">
						<div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-[#A855F7] via-purple-400 to-blue-600" />
						...
						<div className="relative mb-6 mt-4 flex justify-center">
							...
							/>
							<div className="absolute bottom-0 right-10 bg-[#10B981] w-4 h-4 rounded-full border-2 border-[#02040a] z-20 shadow-[0_0_10px_#10B981]" />
						</div>

						<div className="text-center space-y-1 mb-8">
							<h2 className="text-2xl font-['Space_Grotesk'] font-bold tracking-tight text-white">
								{user.name}
							</h2>
							{user.nickname && (
							...
							<div className="inline-flex mt-3 items-center gap-1.5 px-3 py-1 bg-[rgba(168,85,247,0.1)] border border-[rgba(168,85,247,0.3)] text-[#A855F7] rounded-full text-[10px] font-black uppercase tracking-[0.2em]">
								<ShieldCheck size={10} /> Verified Dev
							</div>
						</div>
