...
export async function getUserTokens(userId: string): Promise<{
	...
	const { data, error } = await supabase
		.from("user_tokens")
		.select(
			"github_secret_id, vercel_secret_id, llm_secret_id, llm_base_url, llm_model",
		)
		.eq("user_id", userId)
		.maybeSingle();
	...
	const [githubToken, vercelToken, llmApiKey] = await Promise.all([
		data.github_secret_id
			? readVaultSecret(data.github_secret_id)
			: Promise.resolve(null),
		data.vercel_secret_id
			? readVaultSecret(data.vercel_secret_id)
			: Promise.resolve(null),
		data.llm_secret_id
			? readVaultSecret(data.llm_secret_id)
			: Promise.resolve(null),
	]);
	...
	if (llmApiKey) {
		try {
			const parsed = JSON.parse(llmApiKey);
			if (parsed && typeof parsed === "object") {
				llmApiKeys = parsed as Record<string, string>;
			}
		} catch {
			// Legacy single api key fallback
		}
	}
