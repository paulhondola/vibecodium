...
projectsRoutes.get("/user/:userId", async (c) => {
	try {
		...
		return c.json(
			{ error: `Failed to fetch user projects: ${err.message}` },
			500,
		);
	}
});
...
projectsRoutes.post("/import", async (c) => {
	try {
		...
		if (existing) {
			...
			if (!existingFiles?.length) {
				if (fs.existsSync(targetDir)) {
					...
					await supabase
						.from("projects")
						.update({ status: "ready", local_path: targetDir })
						.eq("id", projectId);
					const allFiles = getAllFilesRecursive(targetDir, targetDir);
					await upsertFiles(projectId, allFiles);
					return c.json(
		...
		const projectName =
			repoUrl.split("/").pop()?.replace(".git", "") || "Untitled";
		...
		fs.mkdirSync("/tmp/vibecodium", { recursive: true });
		const proc = Bun.spawn(["git", "clone", cloneUrl, targetDir], {
			stdout: "pipe",
			stderr: "pipe",
		});
		...
		return c.json(
			{
				success: true,
				message: "Repository imported and indexed",
				projectId,
				name: projectName,
				filesCount: allFiles.length,
			},
...
projectsRoutes.post("/:id/push", async (c) => {
	try {
			...
			return c.json(
				{
					...
					message:
						"You need to register your GitHub Token in your profile to commit and push changes.",
				},
				403,
			);
		}
		...
		const targetDir = await ensureGitRepo(
			projectId,
			project.repo_url,
			githubToken,
		);

		const gitConfigUser = Bun.spawn(
			["git", "config", "user.name", "VibeCodium Live Collaboration"],
			{ cwd: targetDir },
		);
		await gitConfigUser.exited;
		const gitConfigEmail = Bun.spawn(
			["git", "config", "user.email", "live@vibecodium.cloud"],
			{ cwd: targetDir },
		);
		...
		if ((await checkoutProc.exited) !== 0) {
			const checkoutB = Bun.spawn(["git", "checkout", "-b", targetBranch], {
				cwd: targetDir,
				stdout: "pipe",
				stderr: "pipe",
			});
		...
		// This ensures the working tree reflects the latest editor state, but
		// intentionally does NOT touch the git index — the user controls staging
		// via the GitPanel (individual + / - buttons → /api/git add/restore).
		...
		if (commitExit !== 0) {
			...
			return c.json(
				{ error: "Commit failed", details: commitErr || commitOut },
				500,
			);
		}
		...
		const gitPush = Bun.spawn(
			["git", "push", authenticatedUrl, `HEAD:${targetBranch}`, "--force"],
			{
				cwd: targetDir,
				stdout: "pipe",
				stderr: "pipe",
		...
		if (exitCode !== 0)
			return c.json(
				{ error: "Failed to push to GitHub", details: stderr },
				500,
			);
		return c.json({
			success: true,
...
projectsRoutes.post("/:id/branches", async (c) => {
	try {
		...
		if (!/^[a-zA-Z0-9._\-\/]+$/.test(branchName))
			return c.json(
				{ error: "Invalid branch name — use letters, numbers, -, _, /, ." },
				400,
			);
			...
			return c.json(
				{
					...
					error: "GITHUB_TOKEN_REQUIRED",
					message:
						"You need to register your GitHub Token in your profile to push branches.",
				},
				403,
			);
		}
		...
		if (checkoutExit !== 0) {
			return c.json(
				{
					success: false,
					error: checkoutErr.trim() || "Failed to create branch",
				},
				400,
			);
		}
		...
		const pushProc = Bun.spawn(
			["git", "push", "-u", authenticatedUrl, branchName],
			{
				cwd: targetDir,
				stdout: "pipe",
				stderr: "pipe",
		...
		if (pushExit !== 0) {
			return c.json(
				{
					success: false,
					error: pushErr.trim() || "Failed to push branch to GitHub",
				},
				500,
			);
		}
...
projectsRoutes.post("/:id/files/create", async (c) => {
	try {
		...
		if (!projectId || !filePath)
			return c.json({ error: "Missing projectId or path" }, 400);
			...
			return c.json(
				{
					...
					error: "GITHUB_TOKEN_REQUIRED",
					message:
						"Register your GitHub Token in your profile to create repositories.",
				},
				403,
			);
		}
...
projectsRoutes.get("/:id/commits/:sha", async (c) => {
	try {
		...
		if (!ghRes.ok)
			return c.json({ error: `GitHub API error: ${ghRes.statusText}` }, 500);
