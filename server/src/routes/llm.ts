...
llmRoutes.post("/roast", async (c) => {
	try {
		...
		if (!body.code)
			return c.json({ success: false, error: "No code to roast" }, 400);
		...
		const res = await fetch(`${LLM_BASE_URL}/chat/completions`, {
			...
			body: JSON.stringify({
				...
				messages: [
					...
					{
						role: "user",
						content: `Roast this code from file "${body.fileName || "unknown"}":\n\n\`\`\`\n${body.code.slice(0, 3000)}\n\`\`\``,
					},
				],
				max_tokens: 350,
				temperature: 0.9,
...
llmRoutes.get("/ping-llm", async (c) => {
	if (!LLM_KEY)
		return c.json({ success: false, error: "LLM_KEY is not set" }, 500);
	const res = await fetch(`${LLM_BASE_URL}/chat/completions`, {
		method: "POST",
		headers: {
			"Content-Type": "application/json",
	...
	try {
		...
		if (!LLM_KEY)
			return c.json(
				{
					success: false,
					error: "Local LM Studio unreachable and LLM_KEY is not set",
				},
				503,
			);
		try {
