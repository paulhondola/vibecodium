...
router.get("/match", authMiddleware, async (c) => {
	try {
		...
		const excludeIds = [
			...
			...(swiped ?? []).map((s: { swiped_id: string }) => s.swiped_id),
		];

		let baseQuery = supabase
			.from("users")
			.select(
				"id, name, email, picture, bio, language, location, created_at, github_username",
			)
			.limit(20);
...
router.get("/tokens", authMiddleware, async (c) => {
	try {
		...
		const maskedLlmApiKeys: Record<string, string> = {};
		for (const [provider, key] of Object.entries(tokens.llmApiKeys)) {
			if (key) {
				maskedLlmApiKeys[provider] = "****" + key.slice(-4);
			}
		}
...
router.post("/tokens", authMiddleware, async (c) => {
	try {
		...
		// Fetch existing vault secret IDs and decrypted tokens
		const tokens = await getUserTokens(currentUser.sub);
		...
		if (body.llmApiKeys !== undefined) {
			// Merge newly submitted keys with existing ones, skipping masked placeholders
			const mergedKeys = { ...tokens.llmApiKeys };
			for (const [provider, key] of Object.entries(body.llmApiKeys)) {
				if (key === "") {
			...
			if (Object.keys(mergedKeys).length === 0) {
				...
				const serialized = JSON.stringify(mergedKeys);
				const { data: secretId, error } = await supabase.rpc(
					"upsert_vault_secret",
					{
						p_existing_id: existing?.llm_secret_id ?? null,
						p_secret: serialized,
						p_name: `llm_api_keys_${currentUser.sub}`,
					},
				);
				if (error) throw error;
				patch.llm_secret_id = secretId;
			}
		} else if (body.llmApiKey !== undefined) {
			if (body.llmApiKey) {
				const { data: secretId, error } = await supabase.rpc(
					"upsert_vault_secret",
					{
		...
		if (body.llmBaseUrl !== undefined)
			patch.llm_base_url = body.llmBaseUrl || null;
	...
	try {
		...
		if (body.bio !== undefined) {
			if (body.bio.length > 200)
				return c.json(
					{ success: false, error: "Bio must be 200 characters or fewer" },
					400,
				);
		...
		if (body.location !== undefined) {
			if (body.location.length > 100)
				return c.json(
					{ success: false, error: "Location must be 100 characters or fewer" },
					400,
				);
