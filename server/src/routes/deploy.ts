...
deployRoutes.post("/:projectId", async (c) => {
		...
		return c.json(
			{
				...
				message:
					"You need to register your Vercel Token in your profile to deploy websites.",
			},
			403,
		);
	}
	...
	try {
		...
		if (!meRes.ok)
			throw new Error(
				`Invalid VERCEL_TOKEN: ${meData?.error?.message ?? meRes.status}`,
			);
		sendLog(
			`✅ Authenticated as ${meData.user?.email ?? meData.user?.username}`,
		);
		...
		if (!deployRes.ok) {
			console.error(
				"[Deploy] Vercel error response:",
				JSON.stringify(deployData, null, 2),
			);
			const errMsg = deployData?.error?.message ?? JSON.stringify(deployData);
			throw new Error(
				`Vercel deployment failed (${deployRes.status}): ${errMsg}`,
			);
		}
